...
def test_creation():
    ...
    for ii in range(10):
        val = Instance.prop_incremental
        assert it_start == Instance.iterator, 'Lru cache was called again'
    ...
    for ii in range(10):
        val = Instance.prop_incremental
        assert it_start == Instance.iterator - 1, 'LRU-cache was not only called once...'
    ...
    for ii in range(10):
        ...
        val = Instance.prop_incremental
        assert it_start == Instance.iterator, 'LRU-cache was wrongly called.'
...
def test_check_cache():
    ...
    for ii in range(len(vals_nocache)):
        ...
        Instance.value1 = vals_nocache[ii]
        result = Instance.prop_incremental
        assert it_start == Instance.iterator-1, 'LRU-cache was not called.'
...
if (__name__)=="__main__":
    ...
    test_creation()
    print('test_creation completed')
    test_check_cache()
    print('test_check_cache completed')

    # TODO: Maybe check results (might be sufficient to rely on lru_cache)
